...
run :: (Read t, Doodle d, Show k, Ord k, Enum k, Read k, Pool p, Show (d t)) => p k (d t) -> IO ()
run p = prompt "Create a new doodle or participate to an existing one?" (return p) (turn p) >>= run

turn :: (Read t, Doodle d, Ord k, Show k, Enum k, Read k, Pool p, Show (d t)) => p k (d t) -> Either String k -> IO (p k (d t))
...
populate :: (Read t, Doodle d, Show (d t)) => d t -> IO (d t)
